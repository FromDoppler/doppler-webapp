    ...
    },
    {
      ...
      buyUrl: `${
        plan.isFreeAccount
          ...
          : '/buy-push-notification-plans?buyType=5'
      }`,
    },
    {
      addOnType: AddOnType.EcoAI,
      addOnPlan: {
        additional: ecoIA?.plan.additional,
        quantity: ecoIA?.plan.quantity,
        active: ecoIA?.plan.active,
        fee: ecoIA?.plan.fee,
...
export const SubscriptionDetails = InjectAppServices(({ dependencies: { appSessionRef } }) => {
  ...
  const sessionFeatures =
    appSessionRef?.current?.userData?.features ?? appSessionRef?.current?.userData?.user?.features;
  const addOns = getAddons(appSessionRef.current.userData.user, sessionFeatures).filter(
    (a) => a.active,
  );
