...
describe('SelectedPlanChat', () => {
  it('should render SelectedPlanChat when there is not a selected plan chat', async () => {
    ...
    screen.getByText('No se ha seleccionado un plan de chat');

    // click on button
    const button = screen.getByRole('button');
    expect(getByText(button, 'Agregar al carrito')).toBeInTheDocument();
  ...
  it('should render SelectedPlanChat when there is a selected plan chat', async () => {
    ...
    // Assert
    expect(screen.queryByText('No se ha seleccionado un plan de chat')).not.toBeInTheDocument();
    screen.getByRole('list');

    // click on button
    const button = screen.getByRole('button');
    expect(getByText(button, 'Agregar al carrito')).toBeInTheDocument(); // because there is not an added item to cart
  ...
  it('should render SelectedPlanChat when click on remove item button', async () => {
    ...
    const button = screen.getByRole('button');
    expect(getByText(button, 'Remover del carrito')).toBeInTheDocument();
    expect(button).toBeEnabled();
    await userEvent.click(button);
