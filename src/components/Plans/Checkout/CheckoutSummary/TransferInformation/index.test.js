...
describe('TransferInformation', () => {
  ...
  it.each(['ar', 'mx', 'co'])(
    'should render picture 16 variant for supported transfer country %s when upgrade is pending',
    (billingCountry) => {
      const { container } = renderTransferInformation({ billingCountry, upgradePending: true });
      ...
      const exploreLink = screen.getByRole('link', { name: 'Explorar' });
      ...
      expect(
        screen.queryByText(
          /Revisa tu correo, y dentro de las proximas 24 horas hábiles recibirás la factura/i,
        ),
  ...
  it('should keep legacy transfer information for unsupported countries', () => {
    renderTransferInformation({ billingCountry: 'br', upgradePending: true });
