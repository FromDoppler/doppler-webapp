...
export interface DopplerBillingUserApiClient {
  ...
  sendConsultingOfferNotification(data: any): Promise<EmptyResultWithoutExpectedErrors>;
}
...
export class HttpDopplerBillingUserApiClient implements DopplerBillingUserApiClient {
  ...
  public async saveAccountCancellationRequest(
    data: any,
  ): Promise<EmptyResultWithoutExpectedErrors> {
    try {
      const { email, jwtToken } = this.getDopplerBillingUserApiConnectionData();
      ...
      const response = await this.axios.request({
        method: 'POST',
        url: `/accounts/${email}/save-account-cancellation-request`,
        headers: { Authorization: `bearer ${jwtToken}` },
        data: data,
        withCredentials: true,
      });
  ...
  public async setScheduledCancellation(data: any): Promise<EmptyResultWithoutExpectedErrors> {
    try {
      ...
      const response = await this.axios.request({
        method: 'POST',
        url: `/accounts/${email}/set-scheduled-cancellation`,
        headers: { Authorization: `bearer ${jwtToken}` },
        data: data,
        withCredentials: true,
      });
  ...
  public async sendConsultingOfferNotification(
    ...
    try {
      ...
      const response = await this.axios.request({
        method: 'POST',
        url: `/accounts/${email}/send-consulting-offer-notification`,
        headers: { Authorization: `bearer ${jwtToken}` },
        data: data,
        withCredentials: true,
      });
